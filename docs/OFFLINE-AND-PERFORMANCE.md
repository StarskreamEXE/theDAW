...
### 11. DJ deck loading: one decode per file, waveform analysis off the main thread

Loading a DJ deck used to fetch and decode the same file several times over — `djEngine.loadDeck` did its own fetch/decode, and each of the two `DJSemanticWaveform` instances the default layout mounts per deck ran another, each with its own throwaway output `AudioContext`. `frontend/src/lib/djAudioCache.ts` makes it one: a single-flight fetch + `decodeAudioData` **keyed on URL and the decoding context's sample rate** (a buffer decoded at 44.1 kHz is not interchangeable with the same file at 48 kHz), an **LRU of `DJ_AUDIO_CACHE_MAX = 4` decoded buffers** (a 3.5-minute stereo track is ~74 MB of Float32, so the cache is deliberately small: two decks plus one either side of a transition), no defensive `ArrayBuffer` copy, and no real `AudioContext` — decoding runs through the engine's context when one is registered, otherwise a shared `OfflineAudioContext`, which allocates no output device (`djAudioCache.ts`, `getDecodedAudio` / `setDecodeContext`; failures are never cached).

Waveform analysis moved off the main thread in the same pass. The pure half takes `Float32Array`s, derives its bin count from the lane's measured width, memoises per URL + width + rate, and runs in `djSemanticWaveform.worker.ts` with in-flight sharing, the worker receiving copies so the engine's live buffers are never detached (`frontend/src/components/audio/djSemanticWaveformAnalysis.ts`, `analyzeChannels` / `binCountFor` / `analyzeBufferAsync`; rendered waveforms are additionally cached offscreen by `drawWaveformCached`). Measured on a 210 s stereo track with the four waveform instances a deck load mounts: **644.5 ms on the main thread before, 275.2 ms with the synchronous fallback, 41.8 ms with the Worker** — which is what the "lags super bad at the beginning" report was.

Library audio is served with `Cache-Control: private, max-age=31536000` (`backend/modules/library/router.py`), so a deck reload does not pull the whole file down again. Not `immutable`: the browser-unplayable-container transcode cache under `data/playable-cache/` is re-done when a source file is replaced, and a plain `max-age` still skips the download while letting the `FileResponse`'s ETag / Last-Modified answer 304 on a revalidation. `private` because a library belongs to one user: no shared proxy may keep a copy.
...
- **An entry's audio is resolved locally before any CDN.** The library asks the entry's own folder under `data/generations/` first, then an index over the user's own media folders (`theDAW_MEDIA_ROOTS`, or Settings → Storage → Media roots; `backend/modules/library/media_roots.py` `lookup`, consulted from `store.py` `get_audio_path`), and only then a remote copy. A catalogued song whose bytes already sit in a DJ folder plays from that folder, in place — nothing is downloaded and nothing is copied into the library. The containers the browser cannot open (AIFF, WMA, APE) are remuxed to WAV once into `data/playable-cache/<id>/` rather than written beside the user's file (`backend/modules/library/router.py` `_playable_cache_for`), so the referenced folder stays exactly as the user left it.
...
- DJ deck audio fetched+decoded once per URL@sampleRate, single-flight, LRU of 4 decoded buffers, decoded through the engine or an OfflineAudioContext (no output device) -> frontend/src/lib/djAudioCache.ts `getDecodedAudio`, `DJ_AUDIO_CACHE_MAX`, `setDecodeContext`
- DJ waveform analysis runs in a module Worker with in-flight sharing and per-URL+width+rate memoisation; 644.5 ms -> 275.2 ms (sync fallback) -> 41.8 ms (Worker) for the four instances a deck load mounts -> frontend/src/components/audio/djSemanticWaveformAnalysis.ts `analyzeBufferAsync`, `binCountFor`; djSemanticWaveform.worker.ts
- Library audio served with Cache-Control: private, max-age=31536000 (NOT immutable - the playable transcode cache is re-done when a source is replaced; revalidation still 304s) -> backend/modules/library/router.py
- DJ analysis capped at 2 concurrent runs process-wide with single-flight per (entry_id, profile); the deck-only 'dj' profile skips pyin and the second LUFS decode -> backend/modules/analysis/engine.py `MAX_CONCURRENT_ANALYSES`, `PROFILE_DJ`, `analyze_and_persist`
...
- Entry audio resolved entry folder -> media-root index over the user's own folders -> remote copy; referenced in place, never copied -> backend/modules/library/store.py `get_audio_path`; backend/modules/library/media_roots.py `lookup`, `configured_roots`
- Browser-unplayable containers (AIFF/WMA/APE) remuxed once into a derived cache under data/playable-cache, never beside the referenced file -> backend/modules/library/router.py `_playable_cache_for`, `_playable_audio`; media_roots.py `playable_cache_dir`
