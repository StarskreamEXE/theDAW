...
The tab is two columns, each with its own bar along the top at the same
height, so the tab has one row of controls: the writing surface on the left
under **File** and the draft's title, the analysis on the right under its own
bar, with a draggable separator between them.
...
- **On demand.** **ANALYSE** at the right end of the analysis bar runs a full job against the
...
The bar over the writing column is **File** and the title field. The title
field renames the open draft; it is metadata only, never a filename, and it is
the one place the draft's name is shown.

Every move on a draft is in the **File** panel:

| Control | What it does |
|---|---|
| **Drafts** | The switcher. Each row shows the title, a link mark if it is attached to a song, and the line count; the open draft is ticked. Newest first. |
...
| **Delete** | Asks once, in place (**Confirm**, in red), then deletes for good. |

## Attaching a draft to a song

The lower half of the **File** panel is the song half. Pick a song in **Song**
— it starts on the song this draft is already attached to, else the library
selection — and then either direction is one button:

- **Import** starts a *new* draft from that song's existing lyrics. The song
  keeps its own words; you are working on a copy.
- **Save** writes these words into that song's lyrics, through the same path
  SING's own editor takes, so SING and SCORE pick them up immediately and lines
  you did not change keep the timings they already had. It also remembers which
  song this draft became.

An attached draft shows a link mark beside its title, with the song's name in
its tooltip. It is a link, not ownership: the draft stays here, stays editable,
and editing it later does not silently rewrite the song. Save again when you
want the song updated. **Detach** forgets the link and leaves the song with the
words it was given.
