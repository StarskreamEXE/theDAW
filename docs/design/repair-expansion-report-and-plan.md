...
| F03 | Highlight clears when you click away (**corrected 2026-09-18: both guides stated the opposite — "highlight survives click-away" — and the user says that inverted their request**) | P0 | A left-click in the timeline outside a highlight clears it (time range and the clip highlight); inside keeps it; right-click inside opens its menu; Escape clears; one highlight at a time | `WaveformEditor` click paths, `timelineInteraction.highlightClearDecision` |
...
**Corrected 2026-09-18 after reading the code** (the first version of this section guessed wrong).
The "12 stems" option runs `htdemucs_6s` (6 stems), then LARSNET replaces the drum stem with 5 drum
parts (kick, snare, toms, hi-hat, cymbals) **and deletes the combined drum file**. The run therefore
yields **10 files**, not 12. 11 files only appear when the drum file survives, which means LARSNET
failed part-way. theDAW never asks the sidecar for the lead/backing vocal split. The LARSNET parts
are each peak-normalized to 0.98, so they don't sum back to the original drums.

The quality setting already works in the external stem package (`stem-separator-app`); the forced
`shifts=10` was removed there. What is wrong is the app's own text: the dialog's quality hints
describe presets that no longer exist, and quality only changes the model for the 4-stem mode.

- Relabel the option **"Detailed · 10 parts"** and list the exact roles per mode (T24).
- Every run writes a manifest: expected vs produced roles, parts vs aggregates, missing roles,
  `complete / partial / degraded` status. A partial fetch is no longer reported as complete (T23).
- LARSNET's checkpoints are CC BY-NC 4.0. That's fine for this local personal use. The licence is
  recorded in the manifest and shown next to the option.
- "Insert all" excludes aggregates (e.g. a surviving drum mix) unless ticked (follow-up ticket).
- **Deferred to Phase 9:** keeping the drum bus beside its parts and a true 12-part mode both need
  changes in the external package; a python-audio-separator provider for BS-RoFormer SW
  ([MVSEP](https://mvsep.com/algorithms/77),
  [repo](https://github.com/nomadkaraoke/python-audio-separator)) needs its own isolated
  environment.
