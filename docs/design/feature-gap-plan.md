...
| next | batch 8 | **§3.10** step 1 beat-quantized launch on `beatClock` (T13a, #60); **§3.6** step 3b + **§3.8** step 3a the bounce follows the routing graph + render trim (T14); **§3.6** step 3c routing/buses in `.tasmo` (T15); **§3.8** step 3b playhead/meters on the compensated timeline (T16); **§3.7** #40 punch on the loop region (T17) |
