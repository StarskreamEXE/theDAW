...
| 2026-09-15 | `c3d33c8` | **§3.9** `WaveformEditor` on the widget gesture boundary + `automationWrite` removal + hold-only lane slice (T08d-b); **§3.7** #43 step 2 + #07 record press, count-in, engine wiring (T12b-a) then arm button + take meter (T12b-b); **§3.6** step 3a mixer-strips drawer with output pickers, buses, sends (T10c); **§3.5** step 3 render job queue model (T11c-a) then the timeline's flag sites + progress/cancel UI (T11c-b). Run record `docs/runs/P-20260915-batch7.md` |
| 2026-09-15 | `741b79c` | **§3.10** step 1 beat-quantized launch on `beatClock` (T13a, #60); **§3.6** step 3b + **§3.8** step 3a the bounce follows the routing graph + render trim (T14); **§3.6** step 3c routing/buses in `.tasmo` (T15); **§3.8** step 3b playhead/meters on the compensated timeline (T16); **§3.7** #40 punch on the loop region (T17). Run record `docs/runs/P-20260915-batch8.md` |
| next | batch 9 | **§3.8** step 3c offline per-track compensation delays in `renderCore` (T18); **§3.8** step 3d pan and in-chain automation lead by the compensated time (T19); **§3.11** one modulation abstraction `lib/modulation.ts` with NodeF.I. delegating (T20); **§3.12** #49 piano-roll copy/cut/paste (T21); **§3.7** #45 MIDI capture into the armed MIDI track (T22); **§3.12** #47 semantic undo coalescing (T23); wave 2: #46 takes/comping, per-channel metering, #61 follow actions, punch reachability below `2xl` + punch overlay, `EffectKnob` gesture props, `bleed` job kind |
