...
//      takes it off the front, so the SLOWEST path lands where the preview did.
//      Only the slowest: there are no comp delays offline, so every other track
//      prints `maxSec - own` EARLY and the skew BETWEEN two tracks is exactly
//      what it was before T14 — see
//      `theTrimLandsTheSlowestPathAndPrintsEveryOtherTrackSixMsEarly`, which
//      says so in its own assertion. Closing that is a follow-up.
//      The trim also has to DEGRADE with the wiring: a graph that will not
//      order renders with no bus in any path, so it must trim nothing rather
//      than bill a cycle's racks per hop and cut the head off the user's file.
//
...
async function aBusStripIsBuiltAndPlacedByTheGraph(): Promise<void> {
  ...
  // Track tails: t1's panner lands on the bus INPUT, t2's on the master bus,
  // and the bus output lands on the master bus.
  const [t1Pan, t2Pan] = panners(ctx);
  assert.deepEqual(t1Pan.outputs, [busIn], 't1 lands on the bus input, not on the master');
  assert.deepEqual(t2Pan.outputs, [masterBus], 't2 lands on the master');
  ...
  const placed = wireLog.filter((w) => w === `${t1Pan.kind}->${busIn.kind}` || w === `${busOut.kind}->${masterBus.kind}`);
  assert.deepEqual(
    placed, [`${t1Pan.kind}->${busIn.kind}`, `${busOut.kind}->${masterBus.kind}`],
...
async function aCompressorChainTrimsItsDeclaredSixMilliseconds(): Promise<void> {
  ...
  assert.equal(out.length, h.ctxes[0].length, 'and the file is the same length it was rendered at');
}
...
/** The trim is ONE offset on a mix whose tracks are not aligned with each
 *  other: there are no comp delays offline, so at `maxSec` the slowest path
 *  lands exactly and every other track prints `maxSec - own` EARLY. */
async function theTrimLandsTheSlowestPathAndPrintsEveryOtherTrackSixMsEarly(): Promise<void> {
  const h = harness({
  ...
  assert.equal(
    firstSample(out), 6,
    'the whole file moves by the SLOWEST chain — tWet now lands exactly, and tDry, '
      + 'which the compressor never touched and which was exact before, prints 6 ms early. '
      + 'The 6 ms of skew BETWEEN them is what it always was; only which one is right moved. '
      + 'Closing it needs the per-track comp delays live has — a follow-up, not this ticket.',
  );
}
...
async function main(): Promise<void> {
  ...
  await theTrimLandsTheSlowestPathAndPrintsEveryOtherTrackSixMsEarly();
