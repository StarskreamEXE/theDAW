...
export const ContextMenu: React.FC<ContextMenuProps> = ({
  ...
  return createPortal(
    ...
    >
      {title && (
        <div className="px-3 py-1.5 font-display text-xs font-bold uppercase tracking-wider text-zinc-400 border-b border-white/5 mb-0.5 truncate">
          {title}
        </div>
      )}
      {items.map((item, idx) => {
        ...
        if (item.type === 'header') {
          return (
            <div
              key={idx}
              className="px-3 py-1 font-display text-xs font-bold uppercase tracking-wider text-zinc-400"
            >
              {item.label}
            </div>
          );
        ...
        return (
          ...
          >
            ...
            {item.hint != null && (
              <span className="shrink-0 text-zinc-500 normal-case tabular-nums">
                {item.hint}
              </span>
            )}
          </button>
