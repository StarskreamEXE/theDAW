...
export const FEATURES: FeatureEntry[] = [
  ...
  },
  {
    ...
    how: [
      ...
      'Arrange the parts, then EXPORT any part to any format: MusicXML, PDF, ABC, SVG, a note chart or a Beat Saber level.',
    ],
  ...
  },
  {
    ...
    // No 'rhyme': that word belongs to STUDY, whose analysis this tab mounts.
    aliases: ['lyrics', 'writing', 'notepad', 'notebook', 'verse', 'words'],
    guide: 'Bottom Panel Tabs',
  ...
  },
  {
    id: 'panel-details',
    name: 'DETAILS',
    what: 'The selected library item (metadata, prompt, analysis) beside the whole library to browse — or the media bucket for staging loose clips and files',
    how: [
      'Open the DETAILS tab in the bottom dock and click a track in the list on the right.',
      'Read its prompt, metadata and analysis on the left.',
      'Drop audio files from the desktop onto the list to import them into the library.',
      'Switch that column to MEDIA to stage clips and files you have not filed yet.',
    ],
    where: 'Bottom dock → DETAILS',
    surface: { kind: 'dock', tab: 'details' },
    aliases: ['metadata', 'info', 'properties', 'library', 'browse', 'media', 'bucket', 'prompt'],
    guide: 'Details',
  ...
  },
  {
    ...
    how: ['Press the XR Bus button in the VJ toolbar or the SLIDE tab row.', 'Move the simulated controls and watch the bus react.'],
    where: 'VJ toolbar, and the SLIDE tab row',
    surface: { kind: 'center', tab: 'vj' },
  ...
  // ── Inside the panels: controls with a name of their own ───────────────────
  // "meter map" must rank METER MAP (both words) over METER (one exact word),
  // so METER's copy never says "map". featureSearch.test.ts pins the ranking.
  {
    id: 'details-meter-map',
    name: 'METER MAP',
    what: 'A song’s time signature section by section, with its tempo changes, syncopation, swing and polymeter',
    how: [
      'Open the DETAILS tab in the bottom dock and click a track in the library list.',
      'Press MAP in the METER MAP block under the analysis.',
      'Read the time signature, tempo and bars of each section.',
      'Save the result with JSON, or with REPORT for a readable summary.',
    ],
    where: 'Bottom dock → DETAILS',
    surface: { kind: 'dock', tab: 'details' },
    aliases: ['time signature', 'metamorphic meter', 'odd meter', 'rhythm analysis', 'tempo changes', 'polymeter', 'syncopation', 'swing'],
    guide: 'Details',
    locate: tourTarget('meter-map'),
  },
  {
    id: 'midi-meter',
    name: 'METER',
    what: 'The piano roll’s changing time signatures, polymeter lanes and syncopation on one row, with GEN and MATCH',
    how: [
      'Open the MIDI tab in the bottom dock and press METER at the left end of the row under the piano roll.',
      'Step BARS, BEATS and GROUPS for each meter change, and ADD one at the playhead.',
      'Give a lane its own LOOP length, or turn SYNC and ACCENT up.',
      'Press MATCH to take the meter, pickup and tempo from the song in the strip, or GEN to write notes into a lane.',
    ],
    where: 'Bottom dock → MIDI',
    surface: { kind: 'dock', tab: 'midi' },
    aliases: ['match', 'time signatures', 'meter changes', 'polymeter', 'lanes', 'pickup', 'syncopation'],
    guide: 'Piano Roll',
    locate: tourTarget('midi-meter'),
  },
  {
    id: 'sing-study',
    name: 'STUDY',
    what: 'SING’s layout with the lyrics beside their analysis: rhyme classes by vowel colour, assonance, consonance and named devices',
    how: [
      'Select a track, then open the SING tab in the bottom dock.',
      'Press STUDY among the layout keys at the right of the tab row.',
      'Confirm or reject each finding, and add marks of your own.',
    ],
    where: 'Bottom dock → SING',
    surface: { kind: 'dock', tab: 'sing' },
    aliases: ['rhyme', 'rhymes', 'rhyme scheme', 'lyric analysis', 'literary analysis', 'assonance', 'consonance', 'poetry'],
    guide: 'Sing',
    locate: tourTarget('sing-study'),
  },
  {
    // The WEB button is in the analysis pane, which LYRIC always shows and SING
    // shows only in STUDY, so the reveal opens LYRIC.
    id: 'rhyme-web',
    name: 'WEB',
    what: 'A whole lyric drawn as one chart: a node for each line and an arc for each rhyme between them',
    how: [
      'Open the LYRIC tab in the bottom dock, or SING in its STUDY layout.',
      'Analyse the lyric, then press WEB in the control bar above the findings.',
      'Switch between ARC and RING, then save the chart as SVG or PNG.',
    ],
    where: 'Bottom dock → LYRIC, or SING → STUDY',
    surface: { kind: 'dock', tab: 'lyric' },
    aliases: ['rhyme web', 'rhyme chart', 'arc diagram', 'connections'],
    guide: 'Lyric',
    locate: tourTarget('rhyme-web'),
  },
  {
    id: 'score-export',
    name: 'EXPORT menu',
    what: 'Sends the whole sheet or any one part to MusicXML, PDF, ABC, SVG, a note chart or a Beat Saber level',
    how: [
      'Select a track, then open the SCORE tab in the bottom dock and pick a sheet.',
      'Press EXPORT in the toolbar above the sheet.',
      'Choose All parts or one part on the left, then a format on the right.',
    ],
    where: 'Bottom dock → SCORE',
    surface: { kind: 'dock', tab: 'score' },
    aliases: ['score', 'musicxml', 'pdf', 'abc', 'svg', 'beat saber', 'download'],
    guide: 'Exporting scores (the EXPORT menu)',
    locate: tourTarget('score-export'),
  },
  ...
  },
  {
    ...
    how: [
      ...
      'Or press Recent audio beside it to import an audio file theDAW saved or downloaded before.',
    ],
    where: 'Header, right',
    surface: { kind: 'shell' },
