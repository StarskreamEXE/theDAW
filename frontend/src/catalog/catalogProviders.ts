 ...
 * generations, Suno, Udio, an import — or `thedaw` itself, for the things
 * theDAW makes without a model (a DJ performance set, VJ media). There used to
 * be TWO answers to that question — a DETECTED provider the backend reads out of the file's own
...
export const KNOWN_PROVIDERS: Record<string, ProviderMeta> = {
  ...
  // Made IN theDAW, not generated by a model: a DJ performance set, VJ stills
  // and clips, and anything the derivation cannot place. Neutral palette,
  // because it is not an AI generation service.
  thedaw: { id: 'thedaw', label: 'theDAW', color: 'zinc' },
 ...
 *   source === 'generate' / 'studio'     → 'stable-audio'
 *   everything else                      → 'thedaw'
 *
...
export const inferProvider = (e: ProviderEntryFields): string => {
  ...
  // theDAW's native generations, plus a studio bounce of one, are Stable Audio.
  if (e.source === 'generate' || e.source === 'studio') return 'stable-audio';
  // Everything else theDAW makes. This arm used to BE 'stable-audio', which
  // badged every DJ performance set and every VJ still/clip "Stable Audio
  // (AI)" — they are made in theDAW, not generated by a model.
  return 'thedaw';
};
 ...
 * derived id is, except an import, `thedaw` and the unknown bucket — and a
 * slug this app has never heard of is not CLAIMED to be AI on a guess.
 */
